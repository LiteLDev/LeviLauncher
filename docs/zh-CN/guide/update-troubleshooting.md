...
### LeviLauncher 本体一闪即退或双击没反应

- 在 Windows 上，LeviLauncher 现在会以更保守的 WebView2 软件渲染模式启动，以降低部分显卡和驱动组合下的启动崩溃概率。
- 如果界面还没出现就退出，请先检查 `%APPDATA%\levilauncher.exe\logs\startup.log`。
