...
func KeysInOrder(data []byte, encoding Encoding, path []string) ([]string, error) {
    ...
    t := tagType(tByte)
    if !t.IsValid() { return nil, UnknownTagError{Off: r.off, Op: "KeysInOrder", TagType: t} }
    // Read root name if not End
    if t != tagEnd {
        if _, err := encoding.String(r); err != nil { return nil, err }
    }
    // We expect a root compound
    if t != tagStruct { return nil, InvalidTypeError{Off: r.off, FieldType: nil, Field: "<root>", TagType: t} }
...
func keysInCompound(r *offsetReader, encoding Encoding, path []string) ([]string, error) {
    ...
    if len(path) == 0 {
        ...
        for {
            ...
            if err != nil { return nil, err }
            out = append(out, name)
            if err := skipTagValue(r, encoding, tt); err != nil { return nil, err }
        }
        return out, nil
    }
...
func skipTagValue(r *offsetReader, encoding Encoding, tt tagType) error {
    ...
    case tagInt16:
        _, err := encoding.Int16(r); return err
    case tagInt32:
        _, err := encoding.Int32(r); return err
    case tagInt64:
        _, err := encoding.Int64(r); return err
    case tagFloat32:
        _, err := encoding.Float32(r); return err
    case tagFloat64:
        _, err := encoding.Float64(r); return err
    case tagString:
        _, err := encoding.String(r); return err
    case tagByteArray:
        // length followed by bytes
        n, err := encoding.Int32(r); if err != nil { return err }
        _ = r.Next(int(n))
        return nil
    case tagInt32Array:
        n, err := encoding.Int32(r); if err != nil { return err }
    ...
    case tagSlice:
        ...
        listTypeByte, err := r.ReadByte(); if err != nil { return err }
        listType := tagType(listTypeByte)
        if !listType.IsValid() { return UnknownTagError{Off: r.off, Op: "SkipList", TagType: listType} }
        n, err := encoding.Int32(r); if err != nil { return err }
        switch listType {
        case tagByte:
            _ = r.Next(int(n))
        ...
        case tagFloat64:
            ...
            return nil
        case tagString:
            for i := int32(0); i < n; i++ { if _, err := encoding.String(r); err != nil { return err } }
            return nil
        case tagStruct, tagSlice, tagByteArray, tagInt32Array, tagInt64Array:
            for i := int32(0); i < n; i++ { if err := skipTagValue(r, encoding, listType); err != nil { return err } }
            return nil
        default:
            return nil
        }
    case tagStruct:
        ...
        for {
            tByte, err := r.ReadByte(); if err != nil { return err }
            nt := tagType(tByte)
            if nt == tagEnd { break }
            if _, err := encoding.String(r); err != nil { return err }
            if err := skipTagValue(r, encoding, nt); err != nil { return err }
        }
        return nil
    default:
        return nil
