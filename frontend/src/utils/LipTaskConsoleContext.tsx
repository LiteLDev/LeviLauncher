  ...
  return (
    <LipTaskConsoleContext.Provider value={contextValue}>
      ...
      <UnifiedModal
        ...
        footer={
          <>
            <Button onPress={() => void handleCopyLogs()} variant={"secondary"}>
              {t("lip.task_console.copy_logs")}
            </Button>
            <Button
              onPress={() => closeConsole()}
              isDisabled={running}
              variant={closeButtonColor === "danger" ? "danger" : "primary"}
            >
      ...
      >
        <div className="flex h-[min(44vh,26rem)] min-h-[15rem] flex-col gap-3">
          <div className="shrink-0 space-y-3">
            ...
            {errorText ? (
              <div className="rounded-xl border border-rose-300/60 bg-rose-50/60 dark:border-rose-500/30 dark:bg-rose-500/10 px-3 py-2 text-xs text-rose-700 dark:text-rose-300 whitespace-pre-wrap break-all">
                {errorText}
              </div>
            ) : null}
          </div>
          ...
          <div className="min-h-0 flex flex-1 flex-col space-y-1">
            <div className="text-xs font-semibold text-foreground dark:text-zinc-300">
              {t("lip.task_console.log_title")}
            </div>
            <div
              ref={logContainerRef}
              className="min-h-0 flex-1 overflow-y-auto custom-scrollbar rounded-xl border border-border dark:border-zinc-700 bg-surface-secondary/50 dark:bg-zinc-800/60 px-3 py-2"
            >
              {logs.length === 0 ? (
