...
func Setup(ctx context.Context) string {
	...
	wineDir := filepath.Join(utils.GetLauncherConfigRoot(), "wine")
	...
	for {
		...
		if er != nil {
			application.Get().Event.Emit(EventSetupError, "ERR_EXTRACT")
			return "ERR_EXTRACT"
		}
		...
		case tar.TypeReg:
			if err := os.MkdirAll(filepath.Dir(outPath), 0755); err != nil {
				application.Get().Event.Emit(EventSetupError, "ERR_WRITE_FILE")
